...
TCP（Transmission Control Protocol，传输控制协议）是一种面向连接的、可靠的、基于字节流的通信协议，它位于 OSI（Open Systems Interconnection，开放系统互联）模型的传输层。

TCP连接的特性：

1. 面向连接：TCP 通信需要先建立连接，然后才能进行数据传输。连接建立采用三次握手的方式，断开连接采用四次挥手的方式。

2. 可靠性：TCP 提供了确认应答、重传机制、流量控制、拥塞控制等手段，保证了数据的可靠传输。

3. 基于字节流：TCP 把应用程序传给 IP 层的数据看成仅仅是一连串的无结构的字节流，没有边界。

### 2.2 三次握手四次挥手

[三次握手](https://so.csdn.net/so/search?q=三次握手&spm=1001.2101.3001.7020)和四次挥手是各个公司常见的考点，也具有一定的水平区分度，也被一些面试官作为热身题。很多小伙伴说这个问题刚开始回答的挺好，但是后面越回答越冒冷汗，最后就歇菜了。
...
![三次握手和四次挥手.png](D:/%E6%96%87%E4%BB%B6/typora%E5%9B%BE%E7%89%87/format,png.png)
...
![三次握手.png](D:/%E6%96%87%E4%BB%B6/typora%E5%9B%BE%E7%89%87/format,png-1711196020386-1.png)

#### 1.1 为什么需要三次握手，两次不行吗？（字节）
...
![image.png](D:/%E6%96%87%E4%BB%B6/typora%E5%9B%BE%E7%89%87/format,png-1711196020387-2.png)
...
![TCP状态变迁图.jpg](D:/%E6%96%87%E4%BB%B6/typora%E5%9B%BE%E7%89%87/format,png-1711196020387-3.png)
...
在计算机网络中，报文通常指的是一种数据传输的基本单位。每一个报文都包含两部分内容：控制信息和用户数据（负载）。

在HTTP协议中，我们常常把HTTP的请求和响应称为"报文"，它们的基本结构包括：

1. 请求行/状态行：请求行包括请求方法（如GET，POST等）、请求URI和HTTP版本。状态行包括HTTP版本、状态码、和状态描述。

2. 头部字段：包含一系列的头部字段，每个字段都有名值对组成，例如"Host:www.example.com"，"Content-Type:text/html"等。

3. 一个空行：用于区分头部和消息体。

4. 消息体：也就是报文主体，由任意的数据组成，例如一个HTML文档，一个GIF图像等。
...
以上例子是一个简单的GET请求报文，包含了请求行、头部字段（Host, User-Agent, Accept-Language）、以及一个空行。因为是GET请求，所以没有消息体。

请注意，不是所有的HTTP报文都有消息体。例如GET和HEAD请求就没有消息体。

### 2.8 tcp为什么是怎么保证可靠传输的（字节）
